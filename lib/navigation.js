...
var navMethods = function(){
  ...
  function addToTree(menuRoot, pathArr, menuItem){
    ...
    if(pathArr[0]==='.'){
      ...
      //var branch = buildBranch(pathArr, menuItem);
      //menuRoot.items = _.defaults(menuRoot.items, branch);
  ...
  function growBranch(menu, pathArr, menuItem){
    var dirname = pathArr.shift();
    var dirRef;

    // if dir desn't exist, create it
    if(!_(menu).find({title: dirname})){
      dirRef = {
        title: dirname,
        items: []
      };
      ...
      dirRef = _(menu).find({title: dirname});
    }
