...
var navMethods = function(){
  ...
  function addToTree(menuRoot, pathArr, menuItem){
    console.log('addToTree', menuRoot, pathArr, menuItem.linkId);
    // if path array is '.' then just add menu item to root
    if(pathArr[0]==='.'){
  ...
  function growBranch(menu, pathArr, menuItem, linkId){
    //grunt.log.write('pathArr ',pathArr);
    //grunt.log.write('\n   menu ',menu);
    ...
    if(pathArr.length===0){
      ...
      if(menuItem.basename === 'index'){
        ...
        if(!dirRef.items){
          //grunt.log.error(linkId+' dirRef.items not set', dirRef);
          dirRef.items = [];
        }
