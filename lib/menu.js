...
Menu.prototype.placeItem = function (branch, menuPath, parentPath, menuItem) {
  //console.log('placing ' + menuItem.url + ' : ' + parentPath);
  ...
  if (menuPath.length) {
    ...
    var subBranch = _(branch).find(function (o) {
      ...
      //console.log('compare ' + testpath + ' to ' + itemPath);
      return testpath === itemPath;
    });
    var nextPath = path.join(parentPath, parentName);
    if (!subBranch) {
      // create stub item
      subBranch = this.stubMenuItem(branch, nextPath, parentName, menuItem.getView());
    }
...
Menu.prototype.stubMenuItem = function (branch, nextPath, parentName, view) {
  var stubView = new File({
    cwd: view.cwd,
    base: view.base,
    path: path.join(view.base, nextPath, 'index.html')
  });
