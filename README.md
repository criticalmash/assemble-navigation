...
This configuration, will move *main-index* to the top (or left) of the menu. Now say we want to make *Products* the second link and move *Contact* to the end (far right) of the menu. Moving items to the end takes just a little more work, we'll have to specify a *menu_index* value. This is a value that generated by the plug-in and used to sort menu items. But we can set it as well in the config. Here's how it might look...
